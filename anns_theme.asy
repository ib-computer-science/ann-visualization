...
// using shipout(bbox(...)) so exported bitmaps aren't left transparent.
// `unit` fixes the physical size of one drawing-coordinate unit (default
// 1cm), since diagrams are laid out in plain coordinate units and would
// otherwise be shipped out at native point scale (1 unit = 1bp) and come
// out illegibly small.
void renderTheme(picture pic, string filename, Theme theme, real margin=6,
                  real unit=1cm) {
    unitsize(pic, unit);
